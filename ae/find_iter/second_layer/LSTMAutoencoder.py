...
#	enc_state.h = kl_div
        ...
        enc_state = self.enc_state[:,4096:8192]
...
#    self.sparsity_level= np.repeat([0.05], hidden_num).astype(np.float32)
    self.sparsity_level=np.tile([0.01], (self.batch_num, hidden_num)).astype(np.float32)
    kl_div =self.kl_divergence(self.sparsity_level, self.enc_state[:,4096:8192])
    self.sparse_reg = sparse_reg
#    print np.type(enc_state.h)
#    sum_ = np.sum(kl_div)
#    print sum
    self.p_hat = self.enc_state[:,4096:8192]
