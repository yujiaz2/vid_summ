...
def demo(net, image_name, bbox, obj_no, sf_no):
    ...
    filename = 'clipFeat_obj_' + str(obj_no) +'_sf_' + str(sf_no) +'.txt'
    f = open(filename,'a')
...
if __name__ == '__main__':
    ...
    path_frames = '/home/yjzhang/py-faster-rcnn/data/offline1hr/'

    data = sio.loadmat('object_clips_all.mat')
    obj_total_num = int(max(data['object_clips'][:,0]))
    fr_total_num = int(max(data['object_clips'][:,1]))

    for obj_no in range(1,obj_total_num+1):
        if(sum(data['object_clips'][:,0]==obj_no)==0):
            continue
        else:
            clip_current = (data['object_clips'][data['object_clips'][:,0]==obj_no])
            ...
            for sf_no in range(1,sf_num+1):
                ...
                for fr_current_no in [fr_current_start,fr_current_mid,fr_current_end]:
                    ...
                    demo(net, frame, bbox_new, obj_no, sf_no)